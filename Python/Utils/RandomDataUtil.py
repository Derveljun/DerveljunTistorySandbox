...
def randomKoreanLastName():
    ...
    return random.choice(lst_nm_idx)

def randomFromUnicode():
    iNameSize = 3
    chrs = []
    
    for i in range(iNameSize):
        rn = int((random.random() * 11172) + 0xAC00)
        chrs.append(chr(rn))
        
    return ''.join(chrs)
    
def randomCellPhone():
    return '010' + str(int(random.random() * 100000000))
...
def randomCi():
    ...
    return ''.join(random.choice(srting_pool) for _ in range(88))

def randomNumber(size : int):
    string_pool = string.digits
    return ''.join(random.choice(string_pool) for _ in range(size))

if __name__ == "__main__":
    print(randomFromUnicode())
    print(randomKoreanLastName() + randomKoreanFirstName(2))
    print(randomCi())